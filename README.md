...
`navigator.share({ title, text, url, files })`を使ったテキスト、URL、ファイルの共有を確認するためのPoCです。
...
- `navigator.canShare({ title, text, url, files })`が`true`を返すか

タイトル、本文、URL、ファイルはそれぞれ任意で、いずれか1つだけでも共有できます。Web Share APIは、ブラウザだけでなくOSやファイル形式、指定した内容の組み合わせによっても利用可否が異なります。
